	...
	for($i=1;$i<=32; ++$i) {
		...
		if($_POST[$i] != NULL)
		{
			...
			if( $teams[$j] == $_POST[$i] ) {
				...
				$losers[$i] = mysql_real_escape_string($teams[$j+1]);
			}
			else {
				// else the first team must be the loser
				$losers[$i] = mysql_real_escape_string($teams[$j]);
			}
		}
		$j += 2;
	}
	...
	for($i=33;$i<64; ++$i) {
		...
		if($_POST[$i] != NULL)
		{
			...
			if( $_POST[ $childGraph[$i][0] ] == $_POST[$i] ) {
				...
				$losers[$i] = mysql_real_escape_string($_POST[ $childGraph[$i][1] ]);
			}
			else {
				// else the loser must be the first winning child
				$losers[$i] = mysql_real_escape_string($_POST[ $childGraph[$i][0] ]);
			}
		}
	}
